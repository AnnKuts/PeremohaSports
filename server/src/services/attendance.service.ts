...
export class AttendanceService {
  ...
  async createAttendance(sessionId: number, clientId: number) {
    ...
    const isAllowed = await this.attendanceRepository.isClassTypeAllowedInRoom(
      session.room_id,
      session.class_type_id
    );
    if (!isAllowed) {
      throw new AppError(
        'Client cannot be enrolled: the selected activity type is not allowed in this room',
        400
      );
    }

    const hasMembership = await this.attendanceRepository.hasActiveMembershipForClassType(
      clientId,
      session.class_type_id
    );
    if (!hasMembership) {

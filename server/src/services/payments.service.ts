...
export const paymentsService = {
  ...
  async updatePayment(id: number, data: UpdatePaymentInput) {
    ...
    if (data.status === 'completed') {
      const completedCount = await paymentsRepository.countCompletedByClientId(
  ...
  async getRevenueByClassType(year?: number, month?: number) {
    ...
    const revenueMap = new Map<
      string,
      Map<string, { revenue: number; count: number; classTypeId: number }>
    >();
    ...
    const result = Array.from(revenueMap.entries()).map(([month, classTypes]) => ({
      ...
      totalMonthRevenue: Array.from(classTypes.values()).reduce(

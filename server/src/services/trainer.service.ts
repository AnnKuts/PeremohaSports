...
export const TrainersService = {
  ...
  async createTrainer(data: CreateTrainerInput) {
    ...
    if (data.class_type_ids?.length) {
      ...
      if (count !== data.class_type_ids.length)
        throw new AppError('One or more Class Type IDs are invalid', 400);
    }
  ...
  async getTrainerWorkloadStats(trainerId: number) {
    ...
    const result = stats.map((stat) => {
      const typeName =
        typesInfo.find((t) => t.class_type_id === stat.class_type_id)?.name || 'Unknown';
      return {
        class_type: typeName,

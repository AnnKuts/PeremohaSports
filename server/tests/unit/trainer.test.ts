...
describe('TrainersService (unit)', () => {
  ...
  describe('getAllTrainers', () => {
    it('returns array of trainers', async () => {
      vi.mocked(TrainersRepository.findAll).mockResolvedValue([
        { trainer_id: 1, first_name: 'John' }
      ] as any);
  ...
  describe('createTrainer', () => {
    ...
    it('creates trainer successfully', async () => {
      ...
      vi.mocked(TrainersRepository.create).mockResolvedValue({
        trainer_id: 2,
        ...newTrainerData
      } as any);
    ...
    it('throws error if email already exists', async () => {
      vi.mocked(TrainersRepository.findContactByEmail).mockResolvedValue({
        contact_data_id: 1
      } as any);

      await expect(TrainersService.createTrainer(newTrainerData)).rejects.toThrow(
    ...
    it('throws error if gym ids are invalid', async () => {
      ...
      await expect(TrainersService.createTrainer(newTrainerData)).rejects.toThrow(
        'One or more Gym IDs are invalid'
      );
    });
  });
  ...
  describe('updateTrainer', () => {
    ...
    it('updates trainer successfully', async () => {
      ...
      vi.mocked(TrainersRepository.update).mockResolvedValue({
        trainer_id: 1,
        first_name: 'Updated'
      } as any);
    ...
    it('throws error if trainer not found', async () => {
      vi.mocked(TrainersRepository.findById).mockResolvedValue(null);
      await expect(TrainersService.updateTrainer(999, updateData)).rejects.toThrow(
        'Trainer not found'
      );
    });
  });

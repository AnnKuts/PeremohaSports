...
describe('GymService (unit)', () => {
  ...
  it('createGym: throws if room capacity > 200', async () => {
    await expect(
      service.createGym({ address: 'A', rooms: [{ capacity: 201, classTypeIds: [] }] })
    ).rejects.toThrow(AppError);
  });

  it('createGym: throws if room capacity < 1', async () => {
    await expect(
      service.createGym({ address: 'A', rooms: [{ capacity: 0, classTypeIds: [] }] })
    ).rejects.toThrow(AppError);
  });
  ...
  it('getGymUtilizationAnalysis: returns empty if no rooms', async () => {
    mockRepo.findGymsWithUtilizationData = vi.fn(async () => [
      { gym_id: 2, address: 'A', room: [] }
    ]);

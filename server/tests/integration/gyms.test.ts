...
describe('Gyms API Integration', () => {
  ...
  describe('Create & Validation', () => {
    ...
    it('POST /gyms - should create gym with rooms and trainers', async () => {
      ...
      expect(res.body.data.gym.trainer_placement.length).toBe(trainers.length);
    });
  ...
  describe('Read', () => {
    ...
    it('GET /gyms/search - should filter by address', async () => {
      const res = await request(app).get('/gyms/search').query({ address: 'Львів' });

      const body = res.body as ApiResponse<Gym[]>;

      expect(res.status).toBe(200);
      expect(body.data.some((g) => g.address.includes('Львів'))).toBe(true);
    });

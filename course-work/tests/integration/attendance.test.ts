...
describe('Attendance Integration API', () => {
	...
	it('should return 400 if only session_id is provided to /attendance/by-id', async () => {
		const res = await request(app)
			.get('/attendance/by-id?session_id=1');
		expect(res.status).toBe(400);
		expect(res.body).toHaveProperty('error');
	});

	it('should return 400 if only client_id is provided to /attendance/by-id', async () => {
		const res = await request(app)
			.get('/attendance/by-id?client_id=1');
		expect(res.status).toBe(400);
		expect(res.body).toHaveProperty('error');
	});
	...
	it('should not return deleted attendance in list (soft delete)', async () => {
		...
		const res = await request(app)
			.get(`/attendance?session_id=${testSessionId}&client_id=${testClientId}`);
		expect(res.status).toBe(200);
		if (Array.isArray(res.body.data)) {
			const found = res.body.data.find((att: any) => att.session_id === testSessionId && att.client_id === testClientId);
			expect(found).toBeUndefined();
		}
		const prisma = (await import('../../src/lib/prisma')).default;
		const att = await prisma.attendance.findFirst({
			where: { session_id: testSessionId, client_id: testClientId, is_deleted: true }
		});
	...
	it('should fail to update attendance status to invalid value', async () => {
		...
		const res = await request(app)
			...
			.send({ session_id: testSessionId, client_id: testClientId, status: 'not-a-valid-status' });
		expect(res.status).toBe(400);
		expect(res.body).toHaveProperty('error');
	});
	...
	it('should get attendance by session_id and client_id', async () => {
		...
		await request(app)
			...
			.send({ session_id: testSessionId, client_id: testClientId });
		const res = await request(app)
			.get(`/attendance/by-id?session_id=${testSessionId}&client_id=${testClientId}`);

...
describe("AttendanceService (unit)", () => {
  ...
  beforeEach(() => {
    mockRepo = {
      ...
      findById: vi.fn(async (sessionId, clientId) => (sessionId === 1 && clientId === 1 ? { sessionId, clientId, status: "booked" } : null)),
      ...
      getSessionWithRoomAndClassType: vi.fn(async (sessionId) => (sessionId === 1 ? { room_id: 1, class_type_id: 1 } : null)),
      isClassTypeAllowedInRoom: vi.fn(async () => true),
      hasActiveMembershipForClassType: vi.fn(async () => true),
      updateStatus: vi.fn(async (sessionId, clientId, newStatus) => ({ sessionId, clientId, status: newStatus })),
    };
  ...
  it("createAttendance: throws if no active membership", async () => {
    ...
    await expect(service.createAttendance(1, 1)).rejects.toThrow("does not have an active membership");
  });
  ...
  it("updateAttendanceStatus: throws if not found", async () => {
    ...
    await expect(service.updateAttendanceStatus(2, 2, "attended")).rejects.toThrow("Attendance record not found");
  });
  ...
  it("updateAttendanceStatus: throws if already attended and trying to change", async () => {
    ...
    await expect(service.updateAttendanceStatus(1, 1, "booked")).rejects.toThrow("Cannot change status from 'attended'");
  });

  it("updateAttendanceStatus: throws if status is already the same", async () => {
    mockRepo.findById.mockResolvedValueOnce({ sessionId: 1, clientId: 1, status: "booked" });
    await expect(service.updateAttendanceStatus(1, 1, "booked")).rejects.toThrow("Status is already 'booked'");
  });
});
